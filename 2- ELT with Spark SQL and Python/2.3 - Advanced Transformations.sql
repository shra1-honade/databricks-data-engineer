...
Select profile:address:country FROM
(
SELECT customer_id, profile:first_name, profile
FROM customers
where customer_id = 'C00601')
-- NULL ISSUE DUE TO SPARK VERSION I GUESS
...
SELECT customer_id,
  ...
  array_distinct(flatten(collect_set(books.book_id))) as distinct_eles
FROM orders
GROUP BY customer_id
